...
int main()
{
    ...
    */
    ...
    printf("char=> a: %c, the digit number of char %c is %d\n", a, a, a);
    ...
    //double类型
    double money = 0.0;
    printf("double=> money: %lf\n", money);   //要对变量进行初始化.
    printf("double=> money: %.2lf\n", money); //控制限制几位小数点.
    
    //bool类型, 不是原生类型, 其实就是0和1.
    bool flag1 = true;
    bool flag2 = false;
    if (flag1 == true && flag2 == false)
    { //或者直接判断是否等于0和1, 来代替true和false
        printf("bool=> flag1: %d, flag2: %d\n", flag1, flag2);
    }
    else if (false)
    {
        printf("Here is else if.\n");
    }
    else
    {
        printf("Here is else\n");
